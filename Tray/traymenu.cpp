...
TrayMenu::TrayMenu(QWidget *parent)
    ...
    , m_backgroundImage(QWidget::size(), QImage::Format_RGBA8888)
...
{
    TrayMenuItem* addedActionMenuItem = CreateMenuItem(action);
...
TrayMenuItem *TrayMenu::CreateMenuItem(QAction *action)
{
    ...
    else
    {
        ...
        {
            itemToAdd = new TrayMenuItem(action->icon(), this);
        }
        else
        {
            itemToAdd =  new TrayMenuItem(action->icon(), action->text(), this);
        }
    }
