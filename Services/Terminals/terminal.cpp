...
void Terminal::RunConsoleCommandAsync(const QString &command)
{
    m_consoleCommand.last() = command;
    m_process->startDetached(m_terminalCommand, m_consoleCommand);
}

void Terminal::RunConsoleCommandSync(const QString &command, QString &outputInfo, QString &errorInfo)
{
    m_consoleCommand.last() = command;
    m_process->start(m_terminalCommand, m_consoleCommand);
    m_process->waitForFinished(-1);
...
bool Terminal::IsDirExists(const QString folderPath, const char *calledFunc, bool hasRoot)
{
    ...
    else
    {
        ...
        {
            if (QLatin1Literal("Desktop") == pathPart)
            {
...
bool Terminal::IsDirNotExists(const QString folderPath, const char *calledFunc, bool hasRoot)
{
    ...
    else
    {
        ...
        {
            if (QLatin1Literal("Desktop") == subFolderPath)
            {
                const QStringList files = GetFileList(currentPath, calledFunc, hasRoot);
                const QStringList folders = GetFolderList(currentPath, calledFunc, hasRoot);
...
void Terminal::SetPermissionToExecuteSudo(const QString &filePath, const char *calledFunc)
{
    const QString permissionToExecuteCommand = CreatePermissionToExecuteCommandSudo(filePath);
    RunConsoleCommandSync(permissionToExecuteCommand, calledFunc);
}
...
void Terminal::InstallPackageSudo(const QString &packageName, QString &outpiutInfo, QString &errorInfo)
{
    const QString installCommand = CreateInstallPackageCommandSudo(packageName);
    RunConsoleCommandSync(installCommand, outpiutInfo, errorInfo);//это костыль астра кидает ворнинги (из за каких то системных либ)
}
...
QString Terminal::CreateCreateFileCommand(const QString &filePath, bool hasRoot) const noexcept
{
    if (hasRoot)
    {
...
QString Terminal::CreateGetFolderListCommand(const QString &folderPath, bool hasRoot) const noexcept
{
    if (hasRoot)
    {
...
QString Terminal::CreateGetFileListCommand(const QString &folderPath, bool hasRoot) const noexcept
{
    if (hasRoot)
    {
...
QString Terminal::CreateGetFileTextCommand(const QString &filePath, bool hasRoot) const noexcept
{
    if (hasRoot)
    {
...
QString Terminal::CreateAppendTextToFileCommandSudo(const QString &text, const QString &filePath) const noexcept
{
    return QStringLiteral("sudo sh -c \"echo -e \"'") + text + QStringLiteral("'\" >> '") + filePath + QStringLiteral("'\"");
}

QString Terminal::CreateWriteTextToFileCommandSudo(const QString &text, const QString &filePath) const noexcept
{
    return QStringLiteral("sudo sh -c 'echo \"") + text + QStringLiteral("\" >> ") + filePath + '\'';
}

QString Terminal::CreateDeleteFileCommandSudo(const QString &filePath) const noexcept
{
    return QStringLiteral("sudo rm '") + filePath + '\'';
}

QString Terminal::CreateClearFileCommandSudo(const QString &filePath) const noexcept
{
    return QStringLiteral("sudo truncate -s 0 '") + filePath + '\'';
}

QString Terminal::CreateClearFolderCommandSudo(const QString &folderPath) const noexcept
{
    return QStringLiteral("sudo rm -rf ") + folderPath + '*';
}

QString Terminal::CreateDeleteEmptyFolderCommandSudo(const QString &folderPath) const noexcept
{
...
{
    return QStringLiteral("sudo chmod ugo+rwx '") + filePath + '\'';
}

QString Terminal::CreateGettAllProcessListCommand() const noexcept
{
    return QLatin1Literal("ps -eo comm");
}

QString Terminal::CreateKillProcessCommandSudo(const QString &processName) const noexcept
{
    return QStringLiteral("sudo killall ") + processName;
}

QString Terminal::CreateKillProcessCommandSudo(const QString &code, const QString &processId) const noexcept
{
    return QStringLiteral("sudo kill ") + code + ' ' + processId;
}

QString Terminal::CreateGettingAllInstalledPackagesNamesListCommandSudo() const noexcept
{
    return QLatin1Literal("apt-mark showmanual");
}
