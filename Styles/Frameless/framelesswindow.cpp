...
void FramelessWindow::InitUI()
{
    ...
    m_changeDelegatesView->setIcon(QIcon(QStringLiteral(":/images/horizontalDelegateWhite")));
    ...
    m_changeAdditionalSettingsView->setIconSize(QSize(30, 30));
...
void FramelessWindow::checkBorderDragging(QMouseEvent *event)
{
    ...
    if (m_isMousePressed) {
        ...
        if (screenlist.contains(screen)) {
            const QSize sz = QApplication::desktop()->size();
            h = sz.height();
            w = sz.width();
        }
