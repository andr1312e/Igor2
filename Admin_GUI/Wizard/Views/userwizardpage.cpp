...
UserWizardPage::UserWizardPage(WizardService *service, QWidget *parent)
    : MyWizardPage(parent)
    , m_backupTitle(QStringLiteral("Данные администратора хранящиеся в файле восстановления:"))
    , m_oldTitle(QStringLiteral("Данные администратора уже хранящиеся в локальной базе:"))
    , m_noDataTitle(QStringLiteral("Введите данные администратора: ФИО и Звание"))
...
{
    if(m_oldWidget->isVisible())
    {
        if(m_backupWidget->isVisible())
        {
            if(m_actionComboBox->currentIndex()==2)//Восстановление
            {
                return !m_backupWidget->GetUserFCS().remove(' ').isEmpty();
            }
            else//Старые данные
            {
                return !m_oldWidget->GetUserFCS().remove(' ').isEmpty();
            }
        }
        else
        {
...
{
    m_topLabel = new QLabel(QStringLiteral("Данные администратора"));
    m_oldWidget = new UserWizardWidget(m_tableHeader, this);
    m_backupWidget = new UserWizardWidget(m_tableHeader, this);
