...
StartupWizard::StartupWizard(const QString &applicationName, const QString &rlsTiFolder, const LoadingState &loadedDbAdnRolesState, LinuxUserService *linuxUserService, ISqlDatabaseService *iSqlDataBaseService, ThemesNames themeName, QWidget *parent)
    : QWizard(parent)
{
    CreateServices(applicationName, rlsTiFolder, loadedDbAdnRolesState, linuxUserService, iSqlDataBaseService);
    CreateUI(loadedDbAdnRolesState, themeName);
    InitSizes();
    InitStyles();
    InitBehaviour();
    ConnectObjects();
...
void StartupWizard::CreateUI(const LoadingState &loadedDbAdnRolesState, ThemesNames themeName)
{
    m_wizardNavigationBar = new WizardNavigtionBar(this);

    m_introPage = new IntroPage(loadedDbAdnRolesState, m_wizardService, themeName, this);
...
{
    QString message;
    bool hasBackUp = m_wizardService->HasUserData(DbWizardDataType::BackupData) || m_wizardService->HasAnyRolesData(DbWizardDataType::BackupData);
    bool hasOldData = m_wizardService->HasUserData(DbWizardDataType::OldData) || m_wizardService->HasAnyRolesData(DbWizardDataType::OldData);
        ...
        if (hasBackUp) {
            if (hasOldData) {
                message = QStringLiteral("Это страница мастера настроки пользователей программы, их данных и роли.\n"
                                         ...
                                         "Вы можете изменить ФИО, или воиское звание Администратора, в соответсвующих полях\n"
                ...
                message = QStringLiteral("Это страница мастера настроки пользователей программы, их данных и роли.\n"
                                         ...
                                         "При необходимости измените ФИО Администратора, или воиское звание.\n"
                                         "Поля пользователей разрешается отредактировать в интерфейсе программы позже.\n"
                                         "Затем, нажмите кнопку Далее...");
            }
        } else {
            if (hasOldData) {
                message = QStringLiteral("Это страница мастера настроки пользователей программы, их данных и роли.\n"
                                         ...
                                         "при необходимости вы можете изменить ФИО Администратора, или воиское звание.\n"
                ...
                message = QStringLiteral("Поле ввода данных администратора.\n "
                                         ...
                                         "введите пожалуйста ФИО Администратора, и выберите воиское звание снизу.\n"
                                         "Поля других пользователей разрешается отредактировать в интерфейсе программы позже.\n"
                                         "Затем, нажмите кнопку Далее...");
            }
        }
    ...
    case Page_FourthRole:
        if (hasBackUp) {
            if (hasOldData) {
                message = "Данные для программ-ярлыков на рабочем столе, а так же программ которые будут перезапущены для роли: " + Roles.at(currentId() - 2) +
                        "\nВаша программа обнаружила уже имеющиеся данные, так же вы предоставили файл восстановления, "
                        "все эти файлы были загружены.\n"
