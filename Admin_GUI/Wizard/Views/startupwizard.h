...
class StartupWizard : public QWizard
{
   Q_OBJECT
public:
   explicit StartupWizard(const QString &applicationName, const QString &rlsTiFolder, const LoadingState &loadedDbAdnRolesState, LinuxUserService *linuxUserService, ISqlDatabaseService *iSqlDataBaseService,  ThemesNames themeName, QWidget *parent);
   ~StartupWizard();
private:
   ...
   void CreateUI(const LoadingState &loadedDbAdnRolesState, ThemesNames themeName);
   void InitSizes();
   void InitStyles();
   void InitBehaviour();
   void ConnectObjects();
