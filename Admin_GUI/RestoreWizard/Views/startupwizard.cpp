...
StartupWizard::~StartupWizard()
{
    ...
    delete m_filesAndFoldesWizardPage;
...
void StartupWizard::CreateUI(LoadingStates states, ThemesNames themeName)
{
    ...
    m_enviromentWizardPage=new ProfileVariablesWizardPage(m_wizardService->GetEnviromentalVariablesService(), this);
    m_filesAndFoldesWizardPage=new FilesAndFoldersWizardPage(m_wizardService->GetCurrentUserName(), m_wizardService->GetFilesAndFoldesService(), this);
...
void StartupWizard::InitBehaviour()
{
    ...
    setPage(static_cast<int>(WizardPage::Page_FilesAndFolders), m_filesAndFoldesWizardPage);
...
void StartupWizard::OnHelpButtonClick()
{
    ...
    case WizardPage::Page_FilesAndFolders:
    {
        if(hasBackUp)
        {
            message = QStringLiteral("Доп. файлы и папки.\n"
                                     "Файлы и папки представленные в данном окне, копируются вместе с пакетом программ в целевую директорию\n"
                                     "В данном окне помощника с помощью кнопок Добавить папку и Добавить файл допускается добавлять соответствующие сущности\n"
                                     "для их последующего применения при завершении работы мастера");
        }
        else
        {
            message = QStringLiteral("Так как файла восстановления нет, действия в данном окне осуществлены не будут");
        }
        break;
    }
